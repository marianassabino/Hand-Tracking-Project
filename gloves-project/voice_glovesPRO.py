...
volume = 1.0
echo_buffer = np.zeros((int(RATE * 0.3), 1), dtype=np.float32)
echo_pos = 0
...
def input_callback(indata, frames, time, status):
    if status:
        print(status)
    # Convert mono to stereo for output
    stereo = np.column_stack((indata, indata))
    q_in.put(stereo.copy())
...
print("🎤 Starting Voice Gloves PRO (AirPods)...")
...
with sd.InputStream(samplerate=RATE, blocksize=CHUNK,
    ...
    with sd.OutputStream(samplerate=RATE, blocksize=CHUNK,
        ...
        print("✅ AirPods connected! Show your hand!")
        ...
        while True:
            ...
            cv2.imshow("Voice Gloves PRO 🎤", frame)
